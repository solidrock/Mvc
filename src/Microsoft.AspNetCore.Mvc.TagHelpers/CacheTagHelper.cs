    ...
    public class CacheTagHelper : CacheTagHelperBase
    {
        ...
        private async Task<IHtmlContent> ProcessContentAsync(TagHelperOutput output)
        {
            ...
            using (var writer = new CharBufferTextWriter())
            {
                ...
                return new CharBufferHtmlContent(
                    writer.Buffer.Pages,
                    writer.Buffer.CharIndex);
            }
        }
        ...
        {
            private readonly IList<char[]> _content;
            private readonly int _charIndex;

            public CharBufferHtmlContent(IList<char[]> content, int charIndex)
            {
                _content = content;
            ...
            public void WriteTo(TextWriter writer, HtmlEncoder encoder)
            {
                ...
                {
                    var length = i == _content.Count - 1 ?
                        _charIndex :
                        _content[i].Length;

                    writer.Write(_content[i], 0, length);
                }
            }
        }
    }
